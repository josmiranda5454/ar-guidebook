...
2. Configure `CLIMBAR_RECORDER_TOKEN` only in the local Xcode build settings or
   a protected staging build. Use a different token from `CLIMBAR_ADMIN_TOKEN`.
...
The app disables upload when the recorder token is absent. Local snapshots remain
